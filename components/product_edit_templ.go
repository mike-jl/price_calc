...
func IngredientUsageRowEditA() templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		ctx = templ.ClearChildren(ctx)
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 5, "<div class=\"columns\"><div class=\"column\"><div class=\"field\"><label class=\"label is-hidden-tablet product-label\">Ingredient</label><div class=\"control\"><input class=\"input\" type=\"text\" :value=\"usage.ingredient.ingredient.name\" disabled></div></div></div><div class=\"column\"><div class=\"field\"><label class=\"label is-hidden-tablet product-label\">Amount</label><div class=\"field has-addons\"><p class=\"control is-expanded\"><input class=\"input is-fullwidth\" type=\"text\" :form=\"`ingredient-usage-form-${usage.id}`\" :value=\"(usage.quantity * usage.unit.factor).toFixed(2)\" @input=\"usage.quantity = parseFloat($event.target.value) / usage.unit.factor\" name=\"amount\"></p><p class=\"control\"><span class=\"select\"><select :form=\"`ingredient-usage-form-${usage.id}`\" name=\"unit\"><template x-for=\"unit in getFilteredUnitsForUnitId(usage.unit_id)\" :key=\"unit.id\"><option :value=\"unit.id\" x-text=\"unit.name\" :selected=\"unit.id === usage.unit_id\"></option></template></select></span></p></div></div></div><div class=\"column\"><div class=\"field\"><label class=\"label is-hidden-tablet product-label\">Cost</label><div class=\"field has-addons\"><p class=\"control is-expanded\"><input class=\"input\" type=\"text\" :value=\"(usage.ingredient.prices[0].price * usage.quantity).toFixed(2)\" disabled></p><p class=\"control\"><a class=\"button is-static\">€</a></p></div></div></div><form class=\"responsive-buttons form column\" :id=\"`ingredient-usage-form-${usage.id}`\" hx-swap=\"none\" hx-on::after-request=\"alert(&#39;Ingredient usage updated&#39;)\" :hx-post=\"`/ingredient-usage/${usage.id}`\"><button type=\"submit\" class=\"button\">Safe</button> <button class=\"button\" @click=\"cancelEditing(usage)\">Cancel</button> <button class=\"button\" hx-target=\"closest .block\" hx-swap=\"outerHTML\">Delete</button></form></div>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		return nil
